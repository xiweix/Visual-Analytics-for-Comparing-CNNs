...
def visualize_cam(mask, img):
    """Make heatmap from mask and synthesize score-cam result image using heatmap and img.
    Args:
    ...
    Return:
        ...
        result (torch.tensor): synthesized score-cam result of same shape with heatmap.
    """
    img = denormalize(img)
    # mask = standardize_and_clip(mask)
    # heatmap = cv2.applyColorMap(
    #     np.uint8(255 * mask.squeeze().cpu()), cv2.COLORMAP_JET)
    heatmap = cv2.applyColorMap(mask, cv2.COLORMAP_VIRIDIS)
...
def scorecam_to_img(dataset_path, image_name, model_name, output_path):
    ...
    scorecam_map = np.load(os.path.join(output_dir, f'{model_name}_scorecam_mask.npy'))
    score_heat, score_img = visualize_cam(scorecam_map, input_.cpu())
    save_image(score_heat, os.path.join(output_dir, f'{model_name}_scorecam2.png'))
    save_image(score_img, os.path.join(output_dir, f'{model_name}_scorecam_img2.png'))
...
def basic_visualize(input_, gradients, model_name, output_dir, weight=None, cmap='viridis', alpha=0.7):
    ...
    plt.figure('scorecam')
    plt.axis('off')
    plt.imshow(gradients, cmap=None, alpha=None)
    plt.savefig(os.path.join(output_dir, f'{model_name}_scorecam.png'))
    plt.close()

    plt.figure('scorecam_img')
    plt.axis('off')
    for image, cmap, alpha in [(input_, None, None), (gradients, cmap, alpha)]:
        plt.imshow(image, cmap=cmap, alpha=alpha)
    plt.savefig(os.path.join(output_dir, f'{model_name}_scorecam_img.png'))
    plt.close()  
